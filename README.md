...
> **R: Es porque el preprocesador copia y pega todo el contenido de los archivos de biblioteca stdio.h y stdlib.h adentro del código.**
...
> **R: Es porque el preprocesador elimina todos los comentarios para dejar el código limpio antes de compilarlo.**
...
> **R: Con comando normal: (No devuelve nada) .Con -DDEBUG: printf("[DEBUG] %s\n", ("Iniciando main"));**
...
> **R:Estas líneas dicen el número de línea original y el nombre del archivo de donde proviene el código que sigue. El compilador las utiliza para que, si hubiera error,estas puedan informarte la ubicación exacta en tu archivo original (.c o .h) y no en el archivo preprocesado (.i). La declaración de printf proviene del archivo de cabecera stdio.h.**
...
> **R: Estas instrucciones preparar la pila (stack): guardan la posición del programa anterior y reservan espacio de memoria exclusivo para que trabaje la función sumar.**
...
> **R:En programa.o, area_circulo tiene tipo U porque en ese archivo solo se la llama, pero el código fuente no está ahí. Por otro lado, en matematica.o tiene tipo T porque es ahí donde la función está definida.**
...
> **R:Aparece un mensaje de error que dice "Permission denied"**
...
> **R:Quedan porque son símbolos de control del entorno o funciones de bibliotecas externas. El Enlazador no los incluye en el archivo para que el ejecutable sea más liviano. Estos símbolos los resuelve el Cargador del Sistema Operativo al momento de abrir el programa conectándolos con las librerías del sistema.**
...
> **R:La diferencia entre una macro funcion y una funcion real es que una macro función es simplemente un reemplazo de texto; el preprocesador "copia y pega" el código donde sea que se use la macro. Mientras que una función real es un bloque de código separado en la memoria al que salta el programa cuando necesita ejecutarlo.**
...
> **R: La diferencia esta en que contenido almacenan: el símbolo de tipo T (Text) representa instrucciones de código ejecutable, mientras que el símbolo de tipo D (Data) representa variables globales o estáticas ya inicializadas por el programador. Los tipo T viven en la seccion .text mientras que los tipo D en .data**
...
> **R:ntdll.dll => /c/WINDOWS/SYSTEM32/ntdll.dll (0x7ffd56740000)
        KERNEL32.DLL => /c/WINDOWS/System32/KERNEL32.DLL (0x7ffd559d0000)
        KERNELBASE.dll => /c/WINDOWS/System32/KERNELBASE.dll (0x7ffd53010000)
        ucrtbase.dll => /c/WINDOWS/System32/ucrtbase.dll (0x7ffd53de0000)**

¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R:No hace falta especificarla porque GCC la incluye automáticamente por defecto. El compilador asume que cualquier programa en C va a necesitar las funciones básicas (como printf, malloc o exit), por lo que le dice al enlazador que vincule la biblioteca estándar de C sin que el programador tenga que ponerla manualmente.**
